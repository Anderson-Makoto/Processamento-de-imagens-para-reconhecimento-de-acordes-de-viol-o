...
for i in range(0, 64):
    ...
    while True:
        ...
        for cnt in contornos:
            ...
            aprox= cv2.approxPolyDP(cnt, 0*cv2.arcLength(cnt, True), True)
            
            if perimetro> l1 and perimetro< l2:
                print('Tamanho do contorno'+str(len(cnt)))
                ...
                cv2.drawContours(imagem, [aprox], 0, (255, 0, 0), 5)
    ...
    print(str(i)+' feito')
    file.write(str(vet[0])+','+str(vet[1])+','+str(vet[2])+','+str(vet[3])+','+str(vet[4])+','+str(vet[5])+','+str(vet[6])+','+str(vet[7])+','+'A\n')
    print(sum(vet))
