...
file.write('@attribute class {A, B, C, D, E, F, G}\n\n')
file.write('@data\n')
...
for i in range(0, 66):
    ...
    if razao> 2.5:
        ...
        for j in range(0, len(maior)):
            if maior[j][0][0]< pixel_corte:
                cv2.circle(image, (maior[j- 2][0][0], maior[j][0][1]), 5, (0, 0, 0), -1)

            if maior[j][0][0]- pixel_corte< 10 and maior[j][0][0]- pixel_corte> 0 and indice1[0]== 0:
                indice1[0]= maior[j][0][0]
                indice1[1]= maior[j][0][1]

            if maior[j][0][0]- pixel_corte< 10 and maior[j][0][0]- pixel_corte> 0 and (abs(maior[j][0][1]- indice1[1]))> 7:
                indice2[0]= maior[j][0][0]
                indice2[1]= maior[j][0][1]

        print('------'+str(indice1))
        print('------'+str(indice2))
        cv2.line(image, tuple(indice1), tuple(indice2), (255, 255, 255), 3)
    ...
    c_maior= max(contornos, key= cv2.contourArea)
    ...
    file.write(str(vet[0])+','+str(vet[1])+','+str(vet[2])+','+str(vet[3])+','+str(vet[4])+','+str(vet[5])+','+str(vet[6])+','+str(vet[7])+','+str(altura)+','+str(largura)+','+str(maior)+',F\n')
