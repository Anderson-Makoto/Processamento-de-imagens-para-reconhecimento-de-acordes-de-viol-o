...
for i in range(0, 62):
    ...
    if razao> 2.5:
        ...
        for j in range(0, len(maior)):
            if maior[j][0][0]< pixel_corte:
                cv2.circle(image, (maior[j- 2][0][0], maior[j][0][1]), 6, (0, 0, 0), -1)

            if (maior[j][0][0]== pixel_corte) or (maior[j][0][0]== pixel_corte+ 1) and indice[0]== 0:
                print('Entrou no 1')
                indice1[0]= maior[j][0][0]
                indice1[1]= maior[j][0][1]

            elif ((maior[j][0][0]== (pixel_corte+ 2)) or (maior[j][0][0]== (pixel_corte)) or (maior[j][0][0]== (pixel_corte+ 1))):
                print('Entrou no 2')
    ...
    file.write(str(vet[0])+','+str(vet[1])+','+str(vet[2])+','+str(vet[3])+','+str(vet[4])+','+str(vet[5])+','+str(vet[6])+','+str(vet[7])+','+str(altura)+','+str(largura)+','+str(maior)+',D\n')
